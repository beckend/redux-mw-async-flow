...
generateIdMerged=_b.generateId,metaKey=_b.metaKey,metaKeyRequestID=_b.metaKeyRequestID,timeout=_b.timeout;var generateId=generateIdMerged||getGenerateId();var requestStore=new request_store_1.RequestStore;var middleware=function(){return function(next){return function(action){var dispatchNormal=function(){return next(action)};var actionType=action.type;if(!actionType||lGet(action,["meta",metaKey,"enable"])===false)return dispatchNormal();var dispatchAsyncFlow=function(actionArg){mwObservers.before.rootSubject.next(actionArg);
var dispatchResult=next(actionArg);mwObservers.after.rootSubject.next(dispatchResult);return dispatchResult};var metaRequestIdPath=["meta",metaKey,metaKeyRequestID];var metaPromisePath=["meta",metaKey,"promise"];var handleEndAction=function(suffixType,resolve){var requestID=lGet(action,metaRequestIdPath);var theAsyncFlowPromise=lGet(action,metaPromisePath);if(!requestID||!theAsyncFlowPromise)return dispatchNormal();var dispatchResult=dispatchAsyncFlow(action);if(requestID){if(resolve)requestStore.resolve(requestID,
action.payload);else requestStore.reject(requestID,action.payload);var actionEnd=merge({},action,{meta:(_a={},_a[metaKey]={endActionType:actionType,timeEnd:date_1.newDate()},_a),type:async_types_1.replaceSuffix(actionType,suffixType,END)});dispatchAsyncFlow(actionEnd)}else console.warn(action.type+" - meta data not found, did you forget to send it?");return dispatchResult;var _a};if(actionType.endsWith(REQUEST)){var actionClone=cloneDeep(action);var requestID_1=lGet(actionClone,metaRequestIdPath);
...
REQUEST,PENDING)},addedActionMetaData);dispatchAsyncFlow(pendingAction);var newAction=merge({},actionClone,addedActionMetaData);return dispatchAsyncFlow(newAction)}else if(actionType.endsWith(FULFILLED))return handleEndAction(FULFILLED,true);else if(actionType.endsWith(REJECTED))return handleEndAction(REJECTED,false);else if(actionType.endsWith(ABORTED))return handleEndAction(ABORTED,false);else return dispatchNormal();var _b,_c}}};return{middleware:middleware,observers:{after:{obsOnAll:mwObservers.after.obsOnAll,
obsOnEnd:mwObservers.after.obsOnEnd,obsOnRequest:mwObservers.after.obsOnRequest},before:{obsOnAll:mwObservers.before.obsOnAll,obsOnEnd:mwObservers.before.obsOnEnd,obsOnRequest:mwObservers.before.obsOnRequest}}}};