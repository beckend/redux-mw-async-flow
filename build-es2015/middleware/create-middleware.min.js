...
const requestStore=new request_store_1.RequestStore;const middleware=()=>{return(next)=>{return(action)=>{const dispatchNormal=()=>next(action);const dispatchAsyncFlow=(actionArg)=>{mwObservers.before.rootSubject.next(actionArg);const dispatchResult=next(actionArg);mwObservers.after.rootSubject.next(dispatchResult)};const actionType=action.type;if(!actionType||lGet(action,["meta",metaKey,"enable"])===false)return dispatchNormal();const metaRequestIdPath=["meta",metaKey,metaKeyRequestID];const metaPromisePath=
["meta",metaKey,"promise"];const handleEndAction=(suffixType,resolve,payloadArg)=>{const requestID=lGet(action,metaRequestIdPath);const theAsyncFlowPromise=lGet(action,metaPromisePath);if(!requestID||!theAsyncFlowPromise)return dispatchNormal();else dispatchAsyncFlow(action);if(requestID){if(resolve)requestStore.resolve(requestID,payloadArg||action.payload);else requestStore.reject(requestID,payloadArg||action.payload);const actionEnd=merge({},action,{meta:{[metaKey]:{endActionType:actionType,timeEnd:date_1.newDate()}},
...
false);else if(actionType.endsWith(ABORTED))handleEndAction(ABORTED,false);else return dispatchNormal()}}};return{middleware,observers:{after:{obsOnAll:mwObservers.after.obsOnAll,obsOnEnd:mwObservers.after.obsOnEnd,obsOnRequest:mwObservers.after.obsOnRequest},before:{obsOnAll:mwObservers.before.obsOnAll,obsOnEnd:mwObservers.before.obsOnEnd,obsOnRequest:mwObservers.before.obsOnRequest}}}};