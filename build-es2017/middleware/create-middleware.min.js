...
["meta",metaKey,"promise"];const handleEndAction=(suffixType,resolve,payloadArg)=>{const requestID=lGet(action,metaRequestIdPath);const theAsyncFlowPromise=lGet(action,metaPromisePath);if(!requestID||!theAsyncFlowPromise){dispatchNormal();return}else dispatchAsyncFlow(action);if(requestID){if(resolve)requestStore.resolve(requestID,payloadArg||action.payload);else requestStore.reject(requestID,payloadArg||action.payload);const actionEnd=merge({},action,{type:async_types_1.replaceSuffix(actionType,
suffixType,END),meta:{[metaKey]:{endActionType:actionType,timeEnd:date_1.newDate()}}});dispatchAsyncFlow(actionEnd)}else console.warn(`${action.type} - meta data not found, did you forget to send it?`)};if(actionType.endsWith(REQUEST)){const actionClone=cloneDeep(action);let requestID=lGet(actionClone,metaRequestIdPath);if(!requestID||{}.hasOwnProperty.call(requestStore,requestID)){do requestID=generateId({action:actionClone});while({}.hasOwnProperty.call(requestStore,requestID));lSet(actionClone,
metaRequestIdPath,requestID)}const metaTimeoutKey=lGet(actionClone,["meta",metaKey,"timeoutRequest"]);const timeoutRequest=metaTimeoutKey||timeout;const {promise,reject,resolve}=promise_factory_1.createPromise();promise.timeout(timeoutRequest,"timeout").catch((er)=>{reject(er)}).finally(()=>{requestStore.delete(requestID)});const tmpRequestStoreAddPayload={[request_store_1.REQUEST_KEY_PROMISE]:promise,[request_store_1.REQUEST_KEY_RESOLVEFN]:resolve,[request_store_1.REQUEST_KEY_REJECTFN]:reject};requestStore.add(requestID,
tmpRequestStoreAddPayload);const addedActionMetaData={meta:{[metaKey]:{timeout,timeoutRequest,promise,endActionType:null,timeStart:date_1.newDate(),timeEnd:null}}};const pendingAction=merge({},actionClone,{type:async_types_1.replaceSuffix(actionType,REQUEST,PENDING)},addedActionMetaData);dispatchAsyncFlow(pendingAction);const newAction=merge({},actionClone,addedActionMetaData);dispatchAsyncFlow(newAction);return}else if(actionType.endsWith(FULFILLED))handleEndAction(FULFILLED,true);else if(actionType.endsWith(REJECTED))handleEndAction(REJECTED,
false);else if(actionType.endsWith(ABORTED))handleEndAction(ABORTED,false);else dispatchNormal()}}};return{middleware,observers:{before:{obsOnAll:mwObservers.before.obsOnAll,obsOnRequest:mwObservers.before.obsOnRequest,obsOnEnd:mwObservers.before.obsOnEnd},after:{obsOnAll:mwObservers.after.obsOnAll,obsOnRequest:mwObservers.after.obsOnRequest,obsOnEnd:mwObservers.after.obsOnEnd}}}};